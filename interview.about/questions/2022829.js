...
console.log(add({ a: 0.1, b: 0.2 })) //print=>0.30000000000000004
/** 
 **考察点：JS浮点运算问题!
 **/
/* 

    “0.1 + 0.2 = ?” 这个问题，你要是问小学生，他也许会立马告诉你 0.3。但是在计算机的世界里就没有这么简单了，
    做为一名程序开发者在你面试时如果有人这样问你，小心陷阱喽！

你可能在哪里见过 “0.1 + 0.2 = 0.30000000000000004” 但是知道这背后真正的原理吗？是只有 JavaScript 中存在吗？
带着这些疑问本文将重点梳理这背后的原理及浮点数在计算机中的存储机制。

通过本文你能学到什么？
1.浮点数先修知识，更好的帮你理解本文知识
2.IEEE 754 标准是什么？
3.0.1 在 IEEE 754 标准中是如何存储的？
4.0.1 + 0.2 等于多少？
...
0.6 * 2 = 1.2 --------------- 取整数 1，小数 0.2
...
最终 0.1 的二进制表示为 0.00110011... 后面将会 0011 无限循环，因此二进制无法精确的保存类似 0.1 这样的小数。
那这样无限循环也不是办法，又该保存多少位呢？也就有了我们接下来要重点讲解的 IEEE 754 标准。

IEEE 754
IEEE 754 是 IEEE 二进制浮点数算术标准的简称，在这之前各家计算机公司的各型号计算机，有着千差万别的浮点数表示方式，这对数据交换、
计算机协同工作造成了极大不便，该标准的出现则解决了这一乱象，目前已成为业界通用的浮点数运算标准。
...
在 JavaScript 中不论小数还是整数只有一种数据类型表示，这就是 Number 类型，其遵循 IEEE 754 标准，使用双精度浮点数（double）64 位（8 字节）
来存储一个浮点数（所以在 JS 中 1 === 1.0）。其中能够真正决定数字精度的是尾部，即
...
符号 S
符号位的作用是什么？你可能会有此疑惑，在计算机中一切万物都以二进制表示，那么二进制中又以 0 1 存储，你可能想用负号（-）表示负数，
对不起这是不支持的，为了表示负数通常把最高位当作符号位来表示，这个符号位就表示了正负数，0 表示正数（+），1 表示负数（-）。
...
尾数 M
IEEE 754 规定，在计算机内部保存 M 时，默认这个数的第一位总是 1，因此可以被舍去，只保存后面部分，这样可以节省 1 位有效数字，对于双精度 64 位浮点数，
M 为 52 位，将第一位的 1 舍去，可以保存的有效数字为 52 + 1 = 53 位。
...
中间值： 由于科学计数法中的 E 是可以出现负数的，IEEE 754 标准规定指数偏移值的固定值为 ，以双精度浮点数为例：，这个固定值也可以理解为中间值。同理单精度浮点数为 。
...
在 IEEE 754 中，循环位就不能在无限循环下去了，在双精确度 64 位下最多存储的有效整数位数为 52 位，
会采用 就近舍入（round to nearest）模式（进一舍零） 进行存储
...
Python2 的 print 语句会将 0.30000000000000004 转换为字符串并将其缩短为 “0.3”，可以使用 print(repr(.1 + .2)) 获取所需要的浮点数运算结果。
这一问题在 Python3 中已修复。
...
推算 0.1 + 0.2 为什么不等于 0.3 这个过程是乏味和有趣并存的，因为它很难理解，但是一旦你掌握了它，能让你更深刻的认识到其中的存储、运算机制，
从而理解结果为什么是 0.30000000000000004。

最后做个总结，由于计算机底层存储都是基于二进制的，需要事先由十进制转换为二进制存储与运算，这整个转换过程中，类似于 0.1、0.2 这样的数是无穷尽的，
无法用二进制数精确表示。JavaScript 采用的是 IEEE 754 双精确度标准，能够有效存储的位数为 52 位，所以就需要做舍入操作，这无可避免的会引起精度丢失。
另外我们在 0.1 与 0.2 相加做对阶、求和、舍入过程中也会产生精度的丢失。

reference：
"Nodejs技术栈："https://zhuanlan.zhihu.com/p/103254614【知乎】
*/